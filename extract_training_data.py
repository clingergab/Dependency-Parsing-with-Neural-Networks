...
class FeatureExtractor(object):
    ...
    def getWordRep(self, words, pos, word):
        #print(word)
        if pos[word] == 'CD':
            return 0
        elif pos[word] == 'NNP':
            ...
            return 3
        elif words[word].lower() in self.word_vocab:
            return self.word_vocab[words[word].lower()]
        else:
            return 2
    ...
    def get_input_representation(self, words, pos, state):
        ...
        if len(stack) == 0:
            ...
            rep.extend([self.getWordRep(words, pos, stack[-1]), 4, 4])
        elif len(stack) == 2:
            rep.extend([self.getWordRep(words, pos, stack[-1]), self.getWordRep(words, pos, stack[-2]), 4])
        else:
            rep.extend([self.getWordRep(words, pos, stack[-1]), self.getWordRep(words, pos, stack[-2]), self.getWordRep(words, pos, stack[-3])])

        if len(buffer) == 0:
            rep.extend([4, 4, 4])
        elif len(buffer) == 1:
            rep.extend([self.getWordRep(words, pos, buffer[-1]), 4, 4])
        elif len(buffer) == 2:
            rep.extend([self.getWordRep(words, pos, buffer[-1]), self.getWordRep(words, pos, buffer[-2]), 4])
        else:
            rep.extend([self.getWordRep(words, pos, buffer[-1]), self.getWordRep(words, pos, buffer[-2]), self.getWordRep(words, pos, buffer[-3])])
